...
```sql
SHOW TABLES IN lakehouse_mecamecanica.bronze;
...
FROM read_files('/Volumes/lakehouse_mecamecanica/bronze/raw/crm/clientes.csv',
...
WHERE cliente_id IN (SELECT cliente_id FROM read_files(
        '/Volumes/lakehouse_mecamecanica/bronze/raw/crm/clientes.csv',
...
```sql
SHOW TABLES IN lakehouse_mecamecanica.bronze;   -- 10 + _raw_arquivos

WITH contagem AS (
  SELECT 'produtos'     AS tabela, COUNT(*) AS linhas FROM lakehouse_mecamecanica.bronze.produtos
  UNION ALL SELECT 'pedidos',      COUNT(*) FROM lakehouse_mecamecanica.bronze.pedidos
  UNION ALL SELECT 'itens_pedido', COUNT(*) FROM lakehouse_mecamecanica.bronze.itens_pedido
  UNION ALL SELECT 'pagamentos',   COUNT(*) FROM lakehouse_mecamecanica.bronze.pagamentos
  UNION ALL SELECT 'estoque',      COUNT(*) FROM lakehouse_mecamecanica.bronze.estoque
  UNION ALL SELECT 'clientes',     COUNT(*) FROM lakehouse_mecamecanica.bronze.clientes
  UNION ALL SELECT 'vendedores',   COUNT(*) FROM lakehouse_mecamecanica.bronze.vendedores
  UNION ALL SELECT 'carteira',     COUNT(*) FROM lakehouse_mecamecanica.bronze.carteira
  UNION ALL SELECT 'oportunidades',COUNT(*) FROM lakehouse_mecamecanica.bronze.oportunidades
  UNION ALL SELECT 'visitas',      COUNT(*) FROM lakehouse_mecamecanica.bronze.visitas
)
SELECT c.tabela, c.linhas AS na_tabela, r.linhas AS no_arquivo,
       c.linhas = r.linhas AS bate
FROM contagem c
JOIN lakehouse_mecamecanica.bronze._raw_arquivos r ON r.arquivo = c.tabela || '.csv'
...
```sql
DESCRIBE TABLE lakehouse_mecamecanica.bronze.pedidos;
