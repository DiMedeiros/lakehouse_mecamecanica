...
Volume e o job `mecamecanica_pipeline` roda com a primeira tarefa. **Deploy nº 1.**
...
databricks catalogs list --profile projeto-dados-ia | grep mecamecanica || echo 'não existe'
...
SELECT COUNT(*) FROM lakehouse_mecamecanica.bronze.clientes;
-- [TABLE_OR_VIEW_NOT_FOUND] · Catalog 'lakehouse_mecamecanica' was not found
```
...
Crie o projeto da noite 2 em aulas/aula-02-engenharia-de-dados/mecamecanica/,
...
1. databricks.yml
   - bundle name: mecamecanica
   - variables: catalog (default lakehouse_mecamecanica) e warehouse_id
...
6. resources/pipeline.job.yml
   O job mecamecanica_pipeline, com UMA tarefa: raw_conferencia. Serverless.
...
```sql
SHOW SCHEMAS IN lakehouse_mecamecanica;          -- bronze, silver, gold
DESCRIBE VOLUME lakehouse_mecamecanica.bronze.raw;

-- e o COMMENT que o YAML declarou, que é o que documenta a camada
SELECT schema_name, comment
FROM lakehouse_mecamecanica.information_schema.schemata
...
```sql
LIST '/Volumes/lakehouse_mecamecanica/bronze/raw/erp';
LIST '/Volumes/lakehouse_mecamecanica/bronze/raw/crm';
```

```bash
databricks fs ls dbfs:/Volumes/lakehouse_mecamecanica/bronze/raw/erp --profile projeto-dados-ia
```
...
FROM lakehouse_mecamecanica.bronze._raw_arquivos
...
```sql
DROP SCHEMA lakehouse_mecamecanica.gold;    -- ainda está vazio, é seguro fazer ao vivo
SHOW SCHEMAS IN lakehouse_mecamecanica;     -- gold sumiu
```
...
```sql
SHOW SCHEMAS IN lakehouse_mecamecanica;     -- gold voltou idêntico, em segundos
```

**5 · A prova de que a conferência serve para alguma coisa — quebre de propósito**

```bash
databricks fs rm dbfs:/Volumes/lakehouse_mecamecanica/bronze/raw/erp/pagamentos.csv \
