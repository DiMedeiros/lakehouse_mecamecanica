...
FROM lakehouse_mecamecanica.bronze.pedidos;

-- 4 · a quantidade negativa que ninguém explicou para você
SELECT quantidade, COUNT(*) AS itens
FROM lakehouse_mecamecanica.bronze.itens_pedido
...
```sql
SELECT date_trunc('month', to_date(data_pedido)) FROM lakehouse_mecamecanica.bronze.pedidos;
...
- **`try_to_date`, nunca `to_date`.** O Databricks SQL roda em ANSI mode: data
  ...
  FROM lakehouse_mecamecanica.bronze.pedidos GROUP BY 1;
  ...
  FROM lakehouse_mecamecanica.bronze.pedidos GROUP BY 1 ORDER BY 1 LIMIT 5;
...
- **Constraint é contrato, não comentário.** `ALTER TABLE ... ADD CONSTRAINT`
  ...
  ```sql
  CREATE OR REPLACE TABLE lakehouse_mecamecanica.silver._contrato_demo (cnpj STRING);
  ALTER TABLE lakehouse_mecamecanica.silver._contrato_demo
    ADD CONSTRAINT cnpj_14 CHECK (length(cnpj) = 14);

  INSERT INTO lakehouse_mecamecanica.silver._contrato_demo VALUES ('12345678000199'); -- ok
  INSERT INTO lakehouse_mecamecanica.silver._contrato_demo VALUES ('123');
  -- [DELTA_VIOLATE_CONSTRAINT] CHECK constraint cnpj_14 (length(cnpj) = 14) violated

  DROP TABLE lakehouse_mecamecanica.silver._contrato_demo;
  ```
...
```
Continue o bundle em aulas/aula-02-engenharia-de-dados/mecamecanica/.
...
lakehouse_mecamecanica.silver.{tabela}.
...
Escreva o caminho COMPLETO das tabelas no SQL (lakehouse_mecamecanica.silver.x).
...
Rode e me mostre a saída:
  ...
  databricks bundle run mecamecanica_pipeline --target dev --profile projeto-dados-ia
...
```sql
SELECT
  (SELECT COUNT(*) FROM lakehouse_mecamecanica.silver.itens_pedido WHERE devolucao)             AS devolucoes,
  (SELECT COUNT(*) FROM lakehouse_mecamecanica.silver.pedidos      WHERE cancelado)             AS cancelados,
  (SELECT COUNT(*) FROM lakehouse_mecamecanica.silver.itens_pedido WHERE sku_descontinuado)     AS sku_descontinuado,
  (SELECT COUNT(*) FROM lakehouse_mecamecanica.silver.carteira     WHERE orfao_vendedor_desligado) AS carteira_orfa;
```
...
SELECT
  ...
  (SELECT ROUND(SUM(valor_liquido), 2)
     FROM lakehouse_mecamecanica.silver.pedidos)      AS silver_limpa;
...
```sql
SHOW TBLPROPERTIES lakehouse_mecamecanica.silver.clientes;
-- procure as linhas delta.constraints.*  →  o CHECK está gravado na tabela

SHOW TBLPROPERTIES lakehouse_mecamecanica.silver.pedidos;
SHOW TBLPROPERTIES lakehouse_mecamecanica.silver.itens_pedido;
```
...
INSERT INTO lakehouse_mecamecanica.silver.pedidos
...
SELECT COUNT(*) FROM lakehouse_mecamecanica.silver.pedidos
WHERE _processado_em = TIMESTAMP'1999-01-01 00:00:00';   -- 1 · entrou

DELETE FROM lakehouse_mecamecanica.silver.pedidos
