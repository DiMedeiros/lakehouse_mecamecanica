...
```sql
SHOW TABLES IN lakehouse_mecamecanica.gold;   -- nada
```
...
```sql
SELECT
  (SELECT COUNT(*) FROM lakehouse_mecamecanica.silver.itens_pedido)          AS itens_na_silver,
  (SELECT COUNT(*) FROM lakehouse_mecamecanica.gold.fato_vendas)             AS linhas_no_fato,
  (SELECT COUNT(*) FROM lakehouse_mecamecanica.silver.pedidos WHERE cancelado) AS pedidos_cancelados;
-- 197.724 · 191.080 · 957
```
...
FROM lakehouse_mecamecanica.gold.fato_vendas;   -- ~6,9. Se der 13,8, dobrou.
```
...
```sql
SELECT
  (SELECT ROUND(SUM(valor_liquido),2) FROM lakehouse_mecamecanica.silver.pedidos)                   AS silver,
  (SELECT ROUND(SUM(receita),2)       FROM lakehouse_mecamecanica.gold.fato_vendas)                 AS fato,
  (SELECT ROUND(SUM(receita),2)       FROM lakehouse_mecamecanica.gold.mart_vendas_por_vendedor)    AS mart_comercial,
  (SELECT ROUND(SUM(receita),2)       FROM lakehouse_mecamecanica.gold.mart_produto_performance)    AS mart_produto;
